...
for word in words:
    trans_resp = requests.get(
        f"{api_root}/translations", params={"query": word, "src": "fr", "dst": "en"}
    )
    src_resp = requests.get(
        f"{api_root}/external_sources", params={"query": word, "src": "fr", "dst": "en"}
    )
    local_trans = []
...
my_deck = genanki.Deck(2059400110, "French")
for word in words:
    ((src_ex, dst_ex), local_trans) = translations[word]
    my_note = genanki.Note(
        model=my_model, fields=[word, ", ".join(local_trans), src_ex, dst_ex]
    )
